...
describe('i18n Localization Key Coverage', () => {
  ...
  it('should enumerate entity source coverage for later translation work', () => {
    ...
    // Heroic upgraded variants (heroicOf) have no name key: their display name is
    // composed as "Heroic {base}", so they are excluded from the entity manifest.
  ...
  it('should provide every item translation in every locale without canonical fallbacks', () => {
    ...
    // Heroic upgraded variants (heroicOf) carry no name key: their display name is
    // composed as "Heroic {base}" (see itemDisplayName), so they are not in the manifest.
