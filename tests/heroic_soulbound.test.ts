...
describe('soulbound: heroic_mark is bound', () => {
  ...
  it('is filtered out of a trade offer, while an ordinary item is accepted', () => {
    const bags = new Map<number, Map<string, number>>([[1, new Map([['heroic_mark', 5], ['minor_healing_potion', 5]])]]);
    const session: any = { a: 1, b: 2, offerA: null, offerB: null, acceptedA: true, acceptedB: true };
    const ctx = {
    ...
    tradeSetOffer(ctx, [{ itemId: 'heroic_mark', count: 2 }, { itemId: 'minor_healing_potion', count: 2 }], 0, 1);
